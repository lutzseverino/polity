...
class PolityActionAvailabilityServiceTest {
  ...
  private PolityActionAvailabilityService actionAvailability;
  ...
  void setUp() {
    ...
    governmentAssessments =
        new GovernmentAssessmentResolver(
            ...
            procedures);
    actionAvailability =
        new PolityActionAvailabilityService(
            Clock.fixed(NOW.toInstant(), ZoneOffset.UTC),
            access,
            appealProposals,
            appeals,
  ...
  void actionsExposeAvailabilityReasonCodes() {
    ...
    var result = actionAvailability.get(polityId, userId);
  ...
  void provisionalAdmissionAuthorityBelongsOnlyToFounderBeforeFullGovernment() {
    ...
    when(memberships.existsByPolityIdAndUserIdAndStatus(
        ...
        .thenReturn(true);
    when(memberships.existsByPolityIdAndUserIdAndStatus(
            polityId, citizen.getUserId(), MembershipStatus.ACTIVE))
        .thenReturn(true);
    when(membershipService.hasPoliticalStanding(any(UUID.class), any(OffsetDateTime.class)))
        .thenReturn(true);
    assertThat(actionAvailability.hasProvisionalFounderAdmissionAuthority(polityId, founderUserId))
        .isTrue();
    assertThat(
            actionAvailability.hasProvisionalFounderAdmissionAuthority(
                polityId, citizen.getUserId()))
        .isFalse();
  }
