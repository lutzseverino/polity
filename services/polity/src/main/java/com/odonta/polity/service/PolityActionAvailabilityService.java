...
public class PolityActionAvailabilityService {
  ...
  public PolityActionAvailabilityResult get(UUID polityId, UUID userId) {
  ...
  public boolean hasProvisionalFounderAdmissionAuthority(UUID polityId, UUID userId) {
    return hasProvisionalFounderAdmissionAuthority(
        polityId,
        userId,
        memberships.existsByPolityIdAndUserIdAndStatus(polityId, userId, MembershipStatus.ACTIVE));
  }

  public ActionAvailabilityResult sanctionAvailability(UUID polityId, UUID userId) {
    Membership member =
        memberships
            .findEntityByPolityIdAndUserIdAndStatus(polityId, userId, MembershipStatus.ACTIVE)
            .orElseThrow(PolityResource.MEMBER::notFound);
    return sanctionAvailability(member, constitution(polityId));
  }

  private boolean hasProvisionalFounderAdmissionAuthority(Membership member) {
    return hasProvisionalFounderAdmissionAuthority(
        member.getPolityId(), member.getUserId(), member.getStatus() == MembershipStatus.ACTIVE);
  }

  private boolean hasProvisionalFounderAdmissionAuthority(
      UUID polityId, UUID userId, boolean activeMember) {
    Polity polity = polity(polityId);
    ...
    return !governmentAssessments.hasFullGovernmentSize(polityId)
        && polity.getFounderId().equals(userId)
        && activeMember;
  }

  private ActionAvailabilityResult sanctionAvailability(
