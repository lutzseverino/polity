...
public class GovernmentStructureService {
  ...
  public GovernmentStructureResult get(UUID polityId, UUID userId) {
    var constitution = constitutions.get(polityId, userId);
    var polity = polities.findProjectedById(polityId).orElseThrow(PolityResource.POLITY::notFound);
