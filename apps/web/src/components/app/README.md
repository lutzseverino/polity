...
This directory owns conventional wrappers and reusable product components composed
from public `@polity/ui` entrypoints.
...
export it through that folder's `index.ts`, and keep canonical registry source in
`packages/ui/src/components/ui/` untouched.
