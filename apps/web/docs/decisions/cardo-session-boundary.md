...
application shell loader. Every other path restores the owner-local current session and then the separate
Polity account-convergence query before protected shell and route loaders run, redirecting an unrestorable
session to `/sign-in`.
