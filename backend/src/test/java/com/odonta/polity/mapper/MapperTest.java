...
class MapperTest {
  ...
  MapperTest() {
    var conversions = new PolityTransportConversions();
    ReflectionTestUtils.setField(motionTransport, "polityTransportConversions", conversions);
    ReflectionTestUtils.setField(recordTransport, "polityTransportConversions", conversions);
  }
  ...
  void mapsMotionAndVoteAcrossApplicationAndTransportBoundaries() {
    ...
    assertThat(response.getEffectType()).isEqualTo(MotionResponse.EffectTypeEnum.ADOPT_RESOLUTION);
  ...
  void mapsOfficialRecordAcrossApplicationAndTransportBoundaries() {
    ...
    assertThat(response.getType()).isEqualTo("constitution_ratified");
